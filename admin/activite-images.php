...
define('UPLOAD_DIR', __DIR__ . '/../images/activites/');
define('UPLOAD_URL', 'images/activites/');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_id'])) {
    ...
    $r->execute([$img_id, $activite_id]);
    $row = $r->fetch();
    if ($row) {
        $pdo->prepare("DELETE FROM images_activites WHERE id = ?")->execute([$img_id]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['photos'])) {
    ...
    foreach ($_FILES['photos']['tmp_name'] as $k => $tmp) {
        ...
        if ($_FILES['photos']['size'][$k] > MAX_SIZE) {
            $errors[] = htmlspecialchars($_FILES['photos']['name'][$k]) . " : trop lourd (max 20 Mo).";
            continue;
        }
...
<head>
  ...
  <title>Photos — <?= htmlspecialchars($activite['nom_activite']) ?> — Admin</title>
...
<main class="admin-main">
  <div class="admin-topbar">
    <div>
      <h1><i class="fas fa-images" style="color:var(--a-or);margin-right:10px;font-size:1.1rem;"></i>Photos de l'activité</h1>
      <div class="admin-breadcrumb">
        <a href="dashboard.php">Dashboard</a> /
        <a href="activites.php">Activités</a> /
        <a href="activite-form.php?id=<?= $activite_id ?>"><?= htmlspecialchars($activite['nom_activite']) ?></a> /
        Photos
      </div>
    </div>
    <a href="activite-form.php?id=<?= $activite_id ?>" class="btn-admin btn-admin-ghost">
  ...
  <div class="admin-content">
    ...
    <div class="admin-card">
      <div class="admin-card-header">
        <h2>
          <i class="fas fa-compass" style="color:var(--a-or);margin-right:8px;"></i>
